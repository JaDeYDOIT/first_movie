...
// 빈 좌석 데이터 반복문
    var emptySeats = [
        <c:forEach items="${emptylist}" var="empty" varStatus="vs">
            {
                row: ${empty.row},
                col: ${empty.col}
            },
				...
				<div id="PersonSeatCount">
					<div class="select_num_people_wrap">
						...
						<div class="movie_infor">
							...
							<span class="thm">
								<!-- 영화이미지 -->
								<img src="${screenMovieInfo.movie_image}">
							</span>
							<div class="group_infor">
								<div class="bx_tit">
									   <c:choose>
	                                       <c:when test="${screenMovieInfo.movie_audience_rating eq '전체'}">
	                                          <span class="ic_grade gr_all">전체 관람가</span>
	                                       </c:when>
	                                       <c:when test="${screenMovieInfo.movie_audience_rating eq '12세'}">
	                                          <span class="ic_grade gr_12">12세 관람가</span>
	                                       </c:when>
	                                       <c:when test="${screenMovieInfo.movie_audience_rating eq '15세'}">
	                                          <span class="ic_grade gr_15">15세 관람가</span>
	                                       </c:when>
	                                       <c:when test="${screenMovieInfo.movie_audience_rating eq '18세'}">
								...
								<dl>
									...
									<dd class="sub_info1">${screenMovieInfo.formatted_date}
										<span class="time">${screenMovieInfo.formatted_time} ~ ${screenMovieInfo.result_time}</span>
									</dd>
									<dt>영화관</dt>
									<dd class="sub_info1">${screenMovieInfo.branch_name} · ${screenMovieInfo.screen_location}</dd>
								</dl>
							</div>
						</div>
						...
						<div class="count_people">
							...
							<ul>
								...
								<li id="person_12" data-code="12" data-peple="경로" data-count="0"
									value="7000"><strong class="tit">경로</strong> 
									<span class="bx_num">
